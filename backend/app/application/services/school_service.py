...
def _replace_memberships(db: Session, school: School, members_payload: list) -> None:
    ...
    existing_users = db.execute(select(User).where(User.id.in_(user_ids))).scalars().all()
...
def create_school(db: Session, payload: SchoolCreate, creator_user_id: int) -> School:
    ...
    if creator_is_member is None:
        db.add(UserSchoolRole(user_id=creator_user_id, school_id=school.id, role=UserRole.director.value))
    db.commit()
    db.refresh(school)
