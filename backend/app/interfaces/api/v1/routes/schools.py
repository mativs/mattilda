...
@router.put("/{school_id}", response_model=SchoolResponse, dependencies=[Depends(require_school_roles([UserRole.director, UserRole.admin]))])
def update_school_endpoint(
    ...
    updated_school = update_school(db=db, school=school, payload=payload)  # type: ignore[arg-type]
    return serialize_school_response(updated_school)


@router.delete("/{school_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_school_roles([UserRole.director, UserRole.admin]))])
def delete_school_endpoint(
    ...
    if selected_school_id != school_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Path school id must match X-School-Id")
    school = get_school_by_id(db=db, school_id=school_id)
    delete_school(db=db, school=school)  # type: ignore[arg-type]
